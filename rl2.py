...
class HexAgent:
    ...
    def get_action(self, hex_game: Hex, player: int, epsilon: float = 0.1) -> int:
        ...
        self.log_probs.append(
            action_distribution.log_prob(action).to(self.device)
        )  # Ensure log_prob is on device
...
BOARD_SIZE = 11  # Example board size (e.g., 5x5)
            ...
            if current_player == player1:
                ...
                if random.random() < epsilon:
                    ...
                    log_prob_chosen = action_distribution.log_prob(
                        torch.tensor(chosen_move).to(device)
                    )  # Move to device
                else:
